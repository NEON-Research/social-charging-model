...
for sel in subselection1:
    ...
    data = df[mask & (df['week'] >= 42) & (df['charge_points'] <= 20)].copy()
    ...
    data_mean = (
        ...
        .agg({'m_cs': 'mean', 'charge_points': 'mean'})
    ).sort_values('charge_points')

    if data_mean.empty:
        continue

    ax.plot(
        data_mean['charge_points'],
        data_mean['m_cs'],
        label=sel['label'],
        color=sel['color'],
...
ax.set_xlabel('Charge points', fontsize=8)
...
# --- Save plot ---
fig.savefig('plot_charging_satisfaction_CPs.png', bbox_inches='tight', dpi=300)
