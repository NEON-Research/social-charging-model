...
# Two oomcrule values to compare
rule_styles = [
    {'value': True,  'linestyle': '-',  'rule_label': 'oomcrule = True'},
    {'value': False, 'linestyle': ':',  'rule_label': 'oomcrule = False'},
]
...
# --- Format plot ---
ax.set_title('Charging fulfillment ratio\n(sensitivity: oomcrule)', fontsize=9, pad=15)
ax.set_xlabel('EVs per CP', fontsize=8)
ax.tick_params(axis='both', labelsize=8)
ax.set_xlim(1, 15)
ax.set_xticks([5, 10, 15])

# --- Two-part legend: colors for scenarios, linestyles for oomcrule values ---
color_handles = [
    Line2D([0], [0], color=sel['color'], linewidth=2, linestyle='-', label=sel['label'])
    for sel in subselection
]
style_handles = [
    Line2D([0], [0], color='black', linewidth=2, linestyle=rs['linestyle'], label=rs['rule_label'])
    for rs in rule_styles
]
...
# --- Save plot ---
fig.savefig('plot_charging_satisfaction_EVsPerCP_sens_oomcrule.png', bbox_inches='tight', dpi=300)
