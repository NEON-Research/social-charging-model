...
# Three negBiasFactor values to compare
filter_styles = [
    {'value': 2,   'linestyle': '--',  'filter_label': 'negBiasFactor = 2'},
    {'value': 10,  'linestyle': '-', 'filter_label': 'negBiasFactor = 10'},
    {'value': 300, 'linestyle': ':',  'filter_label': 'negBiasFactor = 300'},
]

# Load data
excel_file = 'SCM_results_behaviours_sensNegBias.xlsx'
...
for sel in subselection:
    for fs in filter_styles:
        mask = (
            (df['b1'] == sel['b1']) &
            (df['b2'] == sel['b2']) &
            (df['b3'] == sel['b3']) &
            (df['b4'] == sel['b4']) &
            (df['negBiasFactor'] == fs['value'])
        )
...
# --- Format plot ---
ax.set_title('Charging fulfillment ratio\n(sensitivity: Negativity bias factor to PCP)', fontsize=9, pad=15)
ax.set_xlabel('EVs per CP', fontsize=8)
ax.tick_params(axis='both', labelsize=8)
ax.set_xlim(1, 15)
ax.set_xticks([5, 10, 15])

# --- Two-part legend: colors for scenarios, linestyles for negBiasFactor values ---
color_handles = [
    Line2D([0], [0], color=sel['color'], linewidth=2, linestyle='-', label=sel['label'])
    for sel in subselection
]
style_handles = [
    Line2D([0], [0], color='black', linewidth=2, linestyle=fs['linestyle'], label=fs['filter_label'])
    for fs in filter_styles
]
...
# --- Save plot ---
fig.savefig('plot_charging_satisfaction_EVsPerCP_sens_negBiasFactor.png', bbox_inches='tight', dpi=300)
