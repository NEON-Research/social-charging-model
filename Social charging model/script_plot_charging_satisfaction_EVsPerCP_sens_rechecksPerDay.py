...
# Two rechecksPerDay values to compare
recheck_styles = [
    {'value': 0, 'linestyle': '--',  'recheck_label': 'rechecksPerDay = 0'},
    {'value': 1, 'linestyle': '-',  'recheck_label': 'rechecksPerDay = 1'},
    {'value': 3, 'linestyle': ':',  'recheck_label': 'rechecksPerDay = 3'},
]

# Load data
excel_file = 'SCM_results_behaviours_sensRechecksPerDay.xlsx'
...
for sel in subselection:
    for rs in recheck_styles:
        mask = (
            (df['b1'] == sel['b1']) &
            (df['b2'] == sel['b2']) &
            (df['b3'] == sel['b3']) &
            (df['b4'] == sel['b4']) &
            (df['rechecksPerDay'] == rs['value'])
        )
...
# --- Two-part legend: colors for scenarios, linestyles for rechecksPerDay values ---
color_handles = [
    Line2D([0], [0], color=sel['color'], linewidth=2, linestyle='-', label=sel['label'])
    for sel in subselection
]
style_handles = [
    Line2D([0], [0], color='black', linewidth=2, linestyle=rs['linestyle'], label=rs['recheck_label'])
    for rs in recheck_styles
]
...
# --- Save plot ---
fig.savefig('plot_charging_satisfaction_EVsPerCP_sens_rechecksPerDay.png', bbox_inches='tight', dpi=300)
